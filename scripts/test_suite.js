...
async function launch(s, base) {
  const tx = await s.fac.launchProject("Test", "TST", s.dev.address, s.mkt.address, base === 0 ? ethers.ZeroAddress : base);
...
async function main() {
  ...
  {
    const r = await expectRevert(fac.launchProject("X", "X", dev.address, mkt.address, userA.address));
  ...
  {
    const r = await expectRevert(ft.connect(userA).setFeeSplit(801, 0, 0));
    r ? ok("feeSplit 上限：平台 20% 不可侵占") : bad("feeSplit", "未拦截");
  }

  // ---------- 分红四类 ----------
  {
    await (await fac.configDiv(ftAddr, 1, ethers.ZeroAddress, 0, true)).wait(); // 持币分红 本币
    await (await ft.connect(userA).transfer(userB.address, 10n ** 24n)).wait();
    const bal0 = await ft.balanceOf(userB.address);
    await (await ft.connect(userA).transfer(lp, 10n ** 25n)).wait(); // 卖出触发税→本币切分
  ...
  {
    await (await fac.configDiv(ftAddr, 2, ethers.ZeroAddress, 10n ** 22n, true)).wait(); // 加池分红
    await (await ft.connect(userA).transfer(lp, 10n ** 23n)).wait();
    const sh = await ft.divShares(2, userA.address);
    sh >= 10n ** 23n ? ok("加池分红：转 pair 记份额+门槛") : bad("加池分红", sh.toString());
  }
  {
    await (await fac.configDiv(ftAddr, 3, ethers.ZeroAddress, 0, true)).wait(); // 燃烧分红
    await (await ft.connect(userA).burnDiv(10n ** 23n)).wait();
    const sh1 = await ft.divShares(3, userA.address);
    await (await ft.connect(userA).transfer(DEAD, 10n ** 22n)).wait();
    const sh2 = await ft.divShares(3, userA.address);
    sh1 === 10n ** 23n && sh2 === 10n ** 23n + 10n ** 22n ? ok("燃烧分红：burnDiv + 转黑洞均记份额") : bad("燃烧分红", `${sh1} ${sh2}`);
  }
  {
    await usdt.mint(deployer.address, E("100"));
    await (await usdt.approve(ftAddr, E("100"))).wait();
    await (await fac.configDiv(ftAddr, 2, usdtAddr, 10n ** 22n, true)).wait();
    await (await ft.connect(deployer).depositDivToken(2, usdtAddr, E("10"))).wait();
    ok("自定义 ERC20 分红代币注入（加池分红 USDT 奖励）");
    const u0 = await usdt.balanceOf(userA.address);
    await (await ft.connect(userA).claimDiv(2)).wait();
